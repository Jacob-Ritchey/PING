#!/usr/bin/env bash
# demo.sh — PING protocol walkthrough (Go implementation)
#
# Builds the Go binary, starts two instances on localhost (Blog 1 on :5000,
# Blog 2 on :5001), and walks through the full state machine. Then demonstrates
# the retry/backoff loop by delaying Blog 2's startup.
#
# Usage:
...
for cmd in go curl jq python3; do
...
# GET /inbox is a stable, side-effect-free endpoint on the Go implementation.
wait_for_port() {
...
# Go binary logs to stdout; redirect stdout+stderr so we can tail later.
$BIN 5000 &>"$LOGDIR/blog1.log" &
PIDS+=($!)

info "Starting Blog 2 on :5001"
$BIN 5001 &>"$LOGDIR/blog2.log" &
PIDS+=($!)
...
ok "Ping created: $PING_ID  (Blog 1 state: PING_EXISTS)"

# Allow a moment for the background ping delivery goroutine to reach Blog 2.
sleep 1
...
MISMATCH_RESP=$(curl -s -X POST "$BLOG2/pull/$BAD_ID")
MISMATCH_HTTP=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BLOG2/pull/$BAD_ID" 2>/dev/null || echo "422")
json_pp "$MISMATCH_RESP"
if echo "$MISMATCH_RESP" | jq -e '.error' &>/dev/null; then
    ok "Hash mismatch correctly rejected"
else
    info "Check response above — expected a hash mismatch error"
fi
...
ok "Ping created: $PING_ID2 — sitting in PING_EXISTS, retry loop running"

info "Waiting 12 seconds for the first retry attempt to fail and log…"
sleep 12
...
# The Go retry loop backs off to 10s after the first failure, so Blog 2 should
# receive the ping within ~15s of coming back online.
